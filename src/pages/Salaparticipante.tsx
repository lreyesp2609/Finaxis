...
function evaluateFormula(tokens: FormulaToken[], valoresMap: ValoresMap, anioId: number): number | null {
  ...
  try { const result = Function(`"use strict"; return (${expr})`)(); if (typeof result !== 'number' || !isFinite(result)) return null; return result; }
  catch { return null; }
}
...
function CatalogSheet({ catalogoTab, sala, readOnly, panelVisible, onShowToast }: {
  ...
  const estadoId = catalogoTab.idestadocuenta;
  ...
  useEffect(() => {
    ...
    load();
  }, [estadoId, catalogoTab.idcatalogo]);
  ...
  const handleAddAnio = async () => {
    if (!estadoId || readOnly) return;
    if (!newAnioVal.trim() || anios.some(a => a.valor === newAnioVal.trim())) { onShowToast('Ya existe ese año'); return; }
    setSavingAnio(true);
    const { data: nuevoId, error } = await supabase.rpc('agregar_anio_estado', { p_idestadocuenta: estadoId, p_valor: newAnioVal.trim() });
    if (error || !nuevoId) { onShowToast('Error: ' + (error?.message ?? '')); setSavingAnio(false); return; }
...
export default function SalaParticipante() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (new Date() > new Date(sala.fechafin) || sala.finalizado) {
