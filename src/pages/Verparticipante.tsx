...
interface FormulaActiva {
  id: number;
  nombre: string;
  descripcion: string | null;
  codigo: { tokens: FormulaToken[] };
  source: 'personal' | 'catalogo';
}
...
export default function VerParticipante() {
  ...
  const handleStartCalif = () => {
    setCalifVal(participante && participante.calificacion > 0 ? String(participante.calificacion) : '');
    setCalifError(null);
    setEditingCalif(true);
  };
  ...
  const handleSaveCalif = async () => {
    ...
    if (isNaN(num) || num < 0 || num > 10) {
      setCalifError('Valor entre 0 y 10');
      return;
    }
    setSavingCalif(true);
    setCalifError(null);
  ...
  const handleCancelCalif = () => { setEditingCalif(false); setCalifError(null); };
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: salaData, error: salaErr } = await supabase
        ...
        .eq('codigosala', codigosala!.toUpperCase())
        .eq('user', user!.id)
        .single();

      if (salaErr || !salaData) {
        setError('No tienes permiso para ver esta sala.');
        setLoading(false);
        return;
      }

      setSala({ id: salaData.id, codigosala: salaData.codigosala, sala_nombre: salaData.codigosala, idcatalogo: salaData.idcatalogo, fechainicio: salaData.fechainicio, fechafin: salaData.fechafin });
      ...
      const pInfo: ParticipanteInfo = {
        id: pData.id,
        display_name: pData.display_name ?? pData.email ?? 'Participante',
        email: pData.email ?? '',
        calificacion: pData.calificacion ?? 0,
        idestadocuenta: pData.idestadocuenta,
        idsala: pData.idsala,
      };
      setParticipante(pInfo);

      const estadoId = pInfo.idestadocuenta;

      const { data: rawItems } = await supabase.from('itemcat').select('id, nombre, codigo, contenedor, iditempadre').eq('idcatalogo', salaData.idcatalogo).order('id');
      ...
      const catFormulas: FormulaActiva[] = (rawFec ?? []).map((r: any) => ({ id: r.formula.id, nombre: r.formula.nombre, descripcion: r.formula.descripcion, codigo: r.formula.codigo, source: 'catalogo' }));

      setFormulas([...personales, ...catFormulas]);
      setLoading(false);
    }
        ...
        <div className="vp-header-right">
          ...
          {editingCalif ? (
            <div className="vp-calif-edit">
              <IconStar size={12}/>
              <input
                className="vp-calif-input"
                type="number" min="0" max="10" step="0.1"
                placeholder="0–10"
                value={califVal}
                autoFocus
                onChange={e => { setCalifVal(e.target.value); setCalifError(null); }}
                onKeyDown={e => { if (e.key === 'Enter') handleSaveCalif(); if (e.key === 'Escape') handleCancelCalif(); }}
              />
              ...
              <button className="vp-calif-cancel" onClick={handleCancelCalif}><IconX size={11}/></button>
            </div>
          ) : (
            <button className="vp-calificacion-btn" onClick={handleStartCalif} title="Calificar participante">
              <IconStar size={12}/>
          ...
          <button className={`vp-panel-btn ${panelVisible ? 'vp-panel-btn-active' : ''}`} onClick={() => setPanelVisible(v => !v)}>
            {panelVisible ? 'Ocultar fórmulas' : `Fórmulas${formulas.length > 0 ? ` (${formulas.length})` : ''}`}
          </button>
        </div>
      </div>
          ...
          <div className="vp-formula-panel">
            ...
            {formulas.length === 0 ? (
              <div className="vp-fp-empty">
                ...
                <p>Sin fórmulas</p>
                <span>Este participante no usó fórmulas</span>
              </div>
            ) : (
